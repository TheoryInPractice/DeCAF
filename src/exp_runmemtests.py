...
def run_bsd(out, A, k_input, alg_version, kernel_output, timeout, vertex_indices, groundtruth_cliqs, winf=None,
            perf_lp_v2_enabled=False, num_threads=None):
    '''
    ...
    try:
        if alg_version == 'bsd_dw':  
            ...
            # LP-weights
            B, W, num_lp_runs = bswd_dw_lp.BSWD_DW(A, k_input, perf_lp_v2_enabled, num_threads)
...
def run(pp_dirname, kern_dirname, out_dirname, first_seed, last_seed, k_distinct_max, timeout, typ,
        lp_only, kernel_v2_enabled, perf_lp_v2_enabled, num_threads):
    '''
    ...
    for fname in files: 
        ...
        if run_lp:
            ...
            p = Process(target=run_bsd,
                        args=(out, A, k_distinct, 'bswd_dw_lp', kernel_output,
                              timeout, vertex_indices, groundt_cliqs),
                        kwargs=dict(perf_lp_v2_enabled=perf_lp_v2_enabled, num_threads=num_threads))
            p.start()
            p.join()
            p.close()
...
def main():
    ...
    if kernel_v2_enabled:
        tf_kern_dirname = 'data/kernels_v2/tf/'
        lv_kern_dirname = 'data/kernels_v2/lv/'
    else:
    ...
    out_dir_suffix = {
        ...
        (False, True): '_kv1_pv2',
        (False, False): '',
    }[(kernel_v2_enabled, perf_lp_v2_enabled)]
